...
fn test_in_out_degree_counts_multiplicity() {
    ...
    h.adjacency = vec![
        Hyperedge {
            sources: vec![NodeId(0), NodeId(1), NodeId(1)],
            targets: vec![NodeId(2), NodeId(0)],
        },
        Hyperedge {
            sources: vec![NodeId(2)],
            targets: vec![NodeId(1), NodeId(1)],
        },
    ];

    assert_eq!(h.in_degree(NodeId(0)), 1);
    assert_eq!(h.out_degree(NodeId(0)), 1);
    assert_eq!(h.in_degree(NodeId(1)), 2);
    assert_eq!(h.out_degree(NodeId(1)), 2);
    assert_eq!(h.in_degree(NodeId(2)), 1);
    assert_eq!(h.out_degree(NodeId(2)), 1);
}
